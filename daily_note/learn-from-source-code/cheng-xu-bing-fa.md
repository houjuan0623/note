...
虽然 JS 代码执行是单线程的，但 Node.js 的核心是异步非阻塞 I/O。这意味着当你发起一个 I/O 操作（如读文件、数据库查询、API 请求），主线程不会等待它完成，而是继续执行其他代码，当 I/O 操作完成后，其回调函数会被放入事件队列，等待主线程空闲时执行。

* 问题所在：如果多个异步操作共享并修改同一个状态，这些操作的回调函数执行顺序可能是不确定的。这虽然不是传统意义的“线程安全”问题，但会导致**逻辑上的竞争条件（比如，编写代码的时候 `const res = fetch(url)` 其中fetch是异步函数，但是并没有被 await 所以可能导致下面的程序处理 res 的时候， res 并没有被赋值）**，使得程序状态变得不可预测。
