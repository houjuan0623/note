...
export function ensureRootIsScheduled(root: FiberRootNode) {
  // 调度器不关心是哪个具体组件触发的更新，它只看全局（整个应用）当前最高的优先级是什么
  // 注意：更新的具体内容（Action、State） 是属于某个组件的（存在 FiberNode.updateQueue 中）。但是调度的决策依据（Lanes） 是属于整个应用的（存在 FiberRootNode.pendingLanes 中）。
...
/**
 * 向上遍历找到 Fiber 树的根节点（FiberRootNode）。
 *
 * ### 这个函数的功能是什么？
 * 当在某个具体组件（FiberNode）中触发更新（如调用 `setState`）时，React 需要从这个节点开始，一路向上（通过 `return` 指针）遍历到整棵 Fiber 树的顶端，最终找到全局管理者 `FiberRootNode`，以便将其交给调度中心（Scheduler）进行全局调度。
 *
 * ### 为什么叫 `markUpdateFromFiberToRoot` 这个名字？
 * **在完整的 React 源码中**，这个函数除了“寻找根节点”之外，还承担着一个非常重要的任务：**沿途标记（mark）**。
 * 它会在向上遍历的过程中，把当前更新的优先级（Lane）附加到所有祖先节点的 `childLanes`（或相关优先级属性）上。这样在后续的 Render 阶段，React 从根向下遍历时，就能通过检查 `childLanes` 快速判断某条分支上是否有更新，从而跳过没有变化的子树（这是 React 性能优化的关键）。
 * 尽管在目前的简化实现中，它仅仅做了“寻找并返回 Root”的动作，但为了与 React 官方源码保持一致，依然保留了这个命名。
 * @param fiber 触发更新的起始 Fiber 节点
 * @returns 返回整个应用的根节点 `FiberRootNode`，如果未找到则返回 `null`
 */
