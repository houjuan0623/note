...
1.  TIME\_WAIT 存在的理由1：可靠地实现TCP全双工连接的终止。
    ...
    在上图[#wan-zheng-de-tcp-lian-jie](tcp.md#wan-zheng-de-tcp-lian-jie "mention")中，假设最后一个ACK N+1丢失，服务器将重新发送它最终的那个FIN N，因此客户端必须维护一个状态信息，以允许它重新发送最终的那个ACK。如果没有这个TIME\_WAIT状态，它将响应一个RST，该分节将被服务器解释成一个错误。如果TCP打算彻底终结某个TCP连接上两个方向的数据流，必须保证正确处理连接终止序列中的4个分节中任意一个分节丢失的情况。
