...
>
> 为什么需要三次握手？TCP连接是在一个连接上建立双工通信，每一端都要确定对方收发数据的能力是OK的，所以发送端需要告知接收端，我OK了（发送端发送能力OK），接收端需要告诉发送端（收到），这个确认消息相当于接收端告诉发送端，我的接收能力OK。上面的通信仅能保证发送端可以正常发送数据，接收端能够正常接收数据。同理，接收端和发送端也需要两个消息来保证接收端发送消息正常，发送端接收消息正常。但是由于在初始建立连接的阶段是没有业务数据的，所以将SYN K, ACK J+1合并为一个消息。
