...
接下来我们假设第一个客户终止它的连接，该客户的TCP发送一个FIN，将会使得描述符4变为可读。当服务器读这个已连接套接字时，read将返回0，我们于是关闭该套接字并相应地更新数据结构。把client\[0]置为-1，把描述符集中描述符4的位设置为0。如下图所示：<br>
...
3. **注册回调函数**：服务器调用`epoll_ctl(epfd, EPOLL_CTL_ADD, listen_fd, ...)`。
...
1. 内核必须遍历监控的所有连接（比如 1000 个）。
2. 它去检查每一个连接：“网卡收到这个连接的数据了吗？”
3. 如果没收到，进程挂起。
4. 关键点： 当网卡收到数据触发 CPU 硬件中断时，内核唤醒进程。
5. 进程醒来后，不知道是哪一个连接有数据，只好再次遍历这 1000 个连接来寻找。

结论： 这里硬件（网卡）虽然参与了，但软件层面的算法太笨（O(n) 复杂度），浪费了硬件的中断信号。

### epoll

1. 注册（epoll\_ctl）： 当把一个 socket 加入 `epoll` 时，内核不仅把它存入一棵红黑树（Red-Black Tree），还给这个 socket 绑定了一个回调函数。
2. 硬件介入： 当网卡（硬件）收到数据包，通过 DMA 写入内存，并向 CPU 发出中断信号。
3. 精准打击： CPU 响应中断，执行网卡驱动程序。驱动程序会根据 socket 上的绑定，直接触发之前的回调函数。
4. 入队： 这个回调函数不做别的，只做一件事：把这个有数据的 socket 也就是“就绪”的 socket，直接扔到一个“就绪链表”（Ready List）里。
5. 收割（epoll\_wait）：当程序醒来时，不需要遍历那 1000 个连接，只需要检查这个“就绪链表”里有没有东西。如果有，拿走即可。

结论： `epoll` 是利用硬件产生的中断，精准地只处理活跃的连接。它让硬件中断“告诉”内核谁活了，而不是让内核去“猜”谁活了。
...
无论是select poll epoll，都避免不了在程序层面通过主（死）循环监测数据，但线程休眠的手段可以减少CPU资源的浪费。
...
服务器程序通常需要一个主循环来驱动程序的运行，这个循环可以是事件循环、定时器循环或者信号处理循环。虽然循环会占用一定的CPU资源，但通过使用异步IO、多路复用、硬件加速等技术，不会使无效的循环一直运行，这样一来可以大大减少CPU的消耗，提高服务器程序的性能。
