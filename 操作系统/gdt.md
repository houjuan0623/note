...
| L             |  53 |     1    | 64 位代码段：仅在 IA-32e 模式下使用，在 Linux 0.12 中未使用 |
| AVL           |  52 |     1    | 可用位：由软件使用                                 |
...
在 Linux 0.12 内核的 `head.s` 文件中，定义了如下 GDT 初始化数据：
...
在 Linux 0.12 中，LDT（Local Descriptor Table，局部描述符表）和 TSS（Task State Segment，任务状态段）的描述符都插入在 GDT（Global Descriptor Table，全局描述符表）中。
...
* **L**：64 位代码段，1 位。
  * 仅在 IA-32e 模式下使用，表示段包含 64 位代码。在 Linux 0.12 中未使用。
* **AVL**：可用位，1 位。
...
TSS 描述符是操作系统用于任务切换的重要数据结构，它存储了任务的上下文信息。

**TSS 段描述符结构**

TSS 段描述符也占用 8 个字节，其结构如下表所示：

| 字段            | 位偏移 | 大小 (bit) | 描述                                        |
| ------------- | :-: | :------: | ----------------------------------------- |
| Base\[31:24]  |  56 |     8    | TSS 基址的高 8 位                              |
| G             |  55 |     1    | 粒度（Granularity）：0 表示字节，1 表示 4KB           |
|               |  54 |     1    | 保留，必须为 0                                  |
|               |  53 |     1    | 保留，必须为 0                                  |
| AVL           |  52 |     1    | 可用位：由软件使用                                 |
| Limit\[19:16] |  48 |     4    | TSS 界限的高 4 位                              |
| P             |  47 |     1    | 段存在（Present）：1 表示段存在，0 表示段不存在             |
| DPL           |  45 |     2    | 特权级（Descriptor Privilege Level）：0 \~ 3    |
| S             |  44 |     1    | 描述符类型（Descriptor Type）：0 表示系统段，1 表示代码或数据段 |
| Type          |  40 |     4    | 段类型：9 (386 TSS) 或 11 (可用 386 TSS)         |
| Base\[23:16]  |  32 |     8    | TSS 基址的中间 8 位                             |
| Base\[15:0]   |  0  |    16    | TSS 基址的低 16 位                             |
| Limit\[15:0]  |  16 |    16    | TSS 界限的低 16 位                             |

**字段含义详解**

* **Base**：TSS 基址，32 位，表示 TSS 在内存中的起始地址。
* **G**：粒度，1 位。
  * 0：TSS 界限以字节为单位。
  * 1：TSS 界限以 4KB 为单位。
* **D/B 和 L**：保留位，必须为 0。
* **AVL**：可用位，1 位。
  * 由软件使用，操作系统可以利用此位存储与 TSS 相关的额外信息。
* **Limit**：TSS 界限，20 位。
  * 表示 TSS 的大小，具体单位取决于 G 位的设置。
* **P**：段存在，1 位。
  * 1：TSS 存在于内存中。
  * 0：TSS 不存在于内存中，访问该段会引发异常。
* **DPL**：特权级，2 位。
  * 表示访问该 TSS 所需的最低特权级。
  * 通常设置为 0，表示只有内核可以访问 TSS。
* **S**：描述符类型，1 位。
  * 0：系统段，表示这是一个 TSS 描述符。
* **Type**：段类型，4 位。
  * 9：386 TSS，表示这是一个 386 处理器的 TSS。
  * 11：可用 386 TSS，表示这是一个 386 处理器的 TSS，并且可用。

**Linux 0.12 中的 TSS 初始化**

在 Linux 0.12 内核的 `sched_init` 函数中，会为每个任务设置 TSS 描述符：

```c
set_tss_desc(gdt + FIRST_TSS_ENTRY, &(init_task.task.tss));  // 设置 TSS 描述符
```

* `FIRST_TSS_ENTRY` 是 GDT 中预留的用于存储 TSS 描述符的索引。
* `init_task.task.tss` 是初始任务的 TSS 结构体。
